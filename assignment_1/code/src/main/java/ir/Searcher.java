...
public class Searcher {
    ...
    public PostingsList search( Query query, QueryType queryType, RankingType rankingType, NormalizationType normalizationType ) { 
        ...
        if (query.queryterm.size() > 1 && queryType == QueryType.INTERSECTION_QUERY) {
            ...
            // + 3.4 handle wild card queries
            PostingsList[] tokenDocList = new PostingsList[query.queryterm.size()];
            for (int i = 0; i < query.size(); i++) {
                String token = query.queryterm.get(i).term;   
                if (token.contains("*")) {
                    PostingsList wildcardsPostingsList = new PostingsList();
                    for (Query.QueryTerm qt : kgIndex.handleWildcardQueryTerm(token).queryterm) {
                        // put togehter all postingslists from the wildcard token
                        wildcardsPostingsList = PostingsList.merge(wildcardsPostingsList, this.index.getPostings(qt.term));
                    }
                    // assumption for intersect is a sorted PostingsList
                    Comparator<PostingsEntry> compareByDocId = (PostingsEntry e1, PostingsEntry e2) -> Integer.compare( e1.docID, e2.docID );
                    Collections.sort(wildcardsPostingsList.getList(), compareByDocId);
                    tokenDocList[i] = wildcardsPostingsList;
                } else {
            ...
            for (PostingsList postingsList : tokenDocList) {
                // in case of wildcard, use union (3.4)
            ...
            for (int i = 0; i < query.size(); i++) {
                ...
                if (token.contains("*")) {
                    ...
                    extendedQuery = kgIndex.handleWildcardQueryTerm(token);
                    newQuery.addQueries(extendedQuery);
                } else {
                    newQuery.addToken(token);
                }
            }
            query = newQuery;
            ...
            // 1.2 single word search
            // handle wild card queries
            PostingsList allPostingsCombined = new PostingsList();
            ...
            if (term == "") {
                ...
                if (term.contains("*")) {
                    Query extendedQuery = new Query();
                    extendedQuery = kgIndex.handleWildcardQueryTerm(term);
                    for (int i = 0; i < extendedQuery.queryterm.size(); i++) {
                        String token = query.queryterm.get(i).term;
                        allPostingsCombined = PostingsList.merge(allPostingsCombined, this.index.getPostings(token));
                    }
                    return allPostingsCombined;
                } else {
